...
| Change | Repository |
| --- | --- |
| An NBA LIVE game: codegen config, seeds, game-only kernel stubs, settings, README, research notes | This repository, in the game's folder |
...
long (boot to menus, a Play Now match, a season...). Record anything new you
...
5. Package and publish:
   ...
   ```
   This creates the tag `<GAME>-v<x.y.z>` (for example `LIVE09-v0.1.0`) and
...
Each game is released separately, so an NBA LIVE 09 fix doesn't need a new
NBA LIVE 10 release.
