...
#include "live/input/imgui_gamepad_bridge.h"
#include "live/settings/user_settings_store.h"
#include "live/ui/dialog_layout.h"
...
void SettingsDialog::OnDraw(ImGuiIO& io) {
  ...
  DialogLayout::DrawBackdrop("##live_settings_backdrop", io, 1.0f);
  bool open = true;
  DialogLayout::BeginCenteredPanel("Settings", io, kPanelWidth, &open);

  if (ImGui::BeginTabBar("##live_settings_tabs")) {
...
void SettingsDialog::DrawControlsTab() {
  ...
  CheckboxForCvar("All controllers control player 1", "live_shared_controllers");
